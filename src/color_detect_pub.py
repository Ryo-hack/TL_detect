...
class color_detect :
    def __init__(self):
        ...
        self.output_image_pub = rospy.Publisher('output_image', Image, queue_size=10)
        self.input_image_pub = rospy.Publisher('intput_image', Image, queue_size=10)
        ...
        self.color_red = (255, 0, 0)  # 2値画像で画素値255に割り当てる色
        self.color_blue = (0, 0, 255)  # 2値画像で画素値255に割り当てる色
        self.color_black = (0, 0, 0)  # 2値画像で画素値0に割り当てる色

    def image_callback(self,image_msg):
        input_image = self.bridge.imgmsg_to_cv2(image_msg,"bgr8")
        output_image  = self.process_image(input_image)
        self.output_image_pub.publish(self.bridge.cv2_to_imgmsg(output_image[0], "rgb8"))
        self.input_image_pub.publish(self.bridge.cv2_to_imgmsg(output_image[1], "rgb8"))
    ...
    def process_image(self, frame):
        ...
        dst_red =  np.where(red_img_mask[..., np.newaxis] == 255, self.color_red, self.color_black).astype(np.uint8)
        dst_blue =  np.where(blue_img_mask[..., np.newaxis] == 255, self.color_blue, self.color_black).astype(np.uint8)
